...
- **成本核算** — 上游实时计价优先采信，价格表估算兜底；按日/Key/平台费用报表
- **告警通知** — Key 封禁、平台熔断、服务不可用、配额预警 Webhook 推送
- **出站代理** — 订阅批量拉取、分组健康检查、平台/Key 级绑定（HTTP/SOCKS）
- **管理后台** — 平台、密钥、模型映射、日志、审计的可视化管理，内置 Playground 调试台与失败请求留痕排查
- **两步验证** — 可选 TOTP 动态码登录保护，密钥加密存储
- **定时任务** — Key 用量自动重置、平台模型自动发现、日志自动归档、每日加密备份推送
- **多数据库支持** — D1 / TiDB Cloud / MariaDB / PostgreSQL，运行时自动切换

## 架构

```
用户请求 → 代理入口（Cloudflare 部署：Worker 代理 v1/* + Cron 任务；其他平台：Next.js 路由）
         → 管理后台（Next.js 16 + API 路由）
         → D1 / TiDB / MySQL / MariaDB / PostgreSQL（lib/prisma.ts 统一工厂，按 DB_TYPE 切换）
         → 限流与 Key 封禁状态（Cloudflare 部署存于 KV，其他平台为进程内存储）
```
...
|---------|--------|--------|------|------|
| `d1`（默认） | Cloudflare D1 | `@prisma/adapter-d1` | D1 Binding | Cloudflare |
...
# 使用外部数据库（自备 PostgreSQL / TiDB / MySQL / MariaDB）
docker compose -f docker-compose.standalone.yml up -d --build
```

- 需在 `.env` 中配置数据库密码、`ADMIN_USERNAME` / `ADMIN_PASSWORD`、`JWT_SECRET` 等必填项
- 容器启动时自动同步数据库表结构，管理员账号通过环境变量认证（无 `/setup` 引导页）
- 预构建镜像在每次推送 `canary` 分支时由 **Docker Build** 工作流自动构建（推送 `canary` 标签或手动触发亦可），详细步骤见部署教程
...
| `DB_TYPE` | 数据库类型：`d1`（默认）/ `tidb` / `pg`（Cloudflare 部署）；`mysql` / `mariadb` 仅非 Cloudflare 平台可用 |
| `DATABASE_URL` | 外部数据库 URL（TiDB/MySQL/MariaDB/PostgreSQL 时必需，D1 通过 binding 连接无需设置） |
...
- **缓存**: Cloudflare KV（Cloudflare 部署）
