...
- **Admin dashboard** — Visual management for platforms, keys, logs, and audit trails, with a built-in Playground debugger
...
This project optionally includes **Cloudflare Warp** as a built-in outbound proxy (Docker deployments only, **disabled by default**). Once enabled, requests from this project to upstream LLM providers will be routed through the Cloudflare network, with Cloudflare, Inc. acting as the network provider for that portion of traffic. Such data is processed under the [Cloudflare Privacy Policy](https://www.cloudflare.com/privacypolicy/) and [Terms of Service](https://www.cloudflare.com/terms/). Before enabling, two acknowledgements are required in the admin dashboard ("I understand requests will be routed through the Cloudflare network" and "I have read the Cloudflare Privacy Policy"); enabling is blocked until both are confirmed. **Cloudflare Warp is not affiliated with, sponsored by, or endorsed by this project**, and this project assumes no responsibility for its services.
