...
| 变量 | 说明 | 必填 | 默认值 |
|------|------|------|--------|
| `DB_TYPE` | 数据库类型：`d1` / `tidb` / `mysql` / `mariadb` / `pg` / `hyperdrive` | 是 | `d1`（**仅 Cloudflare 适用**；Vercel/EdgeOne/Docker/Node 必须显式设置为 `tidb`/`mysql`/`mariadb`/`pg`；`hyperdrive` 仅 Cloudflare 场景，PostgreSQL 经 Hyperdrive 连接池加速） |
| `DATABASE_URL` | 数据库连接串（`d1` 不需要） | `tidb`/`mysql`/`mariadb`/`pg` 必须 | — |
...
| `TRUSTED_IP_HEADER` | 自定义可信请求头名称（如 `X-Real-IP`、`X-Client-IP` 或网关注入的任意自定义头）。**当反代/网关会强制覆盖某个特定请求头为真实客户端 IP（客户端无法伪造）时配置**：配置后该头优先级最高（优先于 `DEPLOY_PLATFORM` 平台方案与 `TRUSTED_PROXY_IPS` 信任链），登录限流与审计日志直接采信其值；该头缺失或为空时回退平台方案。**仅在确认上游会强制覆盖该头时配置**——否则攻击者可直接伪造该头绕过限流 | 否 | 空 |
| `CSRF_ALLOWED_ORIGINS` | 来源域名白名单（逗号分隔，填域名，不带协议与端口）。**CDN 回源时源站收到的请求域名是源站域名、与浏览器访问的 CDN 域名不一致，导致登录等写操作被来源校验拒绝时配置**：填入浏览器实际访问的域名。未配置时按默认校验（来源必须与源站收到的请求域名一致），直连部署无需配置 | 否 | 空 |
...
| `NODE_NAME` | 节点名称：同一项目多实例部署（如 CDN 后多源站）时填写，用于在请求日志中区分请求来自哪个实例。名称中的换行、逗号、引号等特殊字符会被自动移除，超长自动截断（中文最多 10 字、英文最多 20 字）。未设置时自动使用部署平台名（EdgeOne/Vercel/Docker/Cloudflare，均未设置时为 local） | 否 | 部署平台名 |
...
::: warning
`DB_TYPE` 必须与实际数据库一致：`tidb` 配 MySQL 协议连接串，`mysql` 配 `mysql://` 连接串，`mariadb` 配 `mariadb://` 连接串，`pg` 配 PostgreSQL 协议连接串
:::
