...
| 方式 | 架构 | 数据库 | 定时任务 |
|------|------|--------|----------|
| **Cloudflare Pages + Worker** | Worker 处理代理与定时任务，Pages 托管前台与管理后台 | D1（免费，零配置）或 TiDB/PG | Cloudflare 内置（免费） |
| **Vercel / EdgeOne** | 服务端函数处理一切 | TiDB / PostgreSQL（远程） | HTTP 端点 + 外部调度（Vercel Cron 需 Pro 计划） |
| **Node.js 直接部署** | 自有服务器跑完整服务 | TiDB / PostgreSQL | HTTP 端点 + 系统 cron |
| **Docker** | 容器化部署 | PostgreSQL / MySQL | HTTP 端点 + 系统 cron |
...
| 数据库 | D1（默认，零配置） | TiDB/PG（远程） | TiDB/PG（远程） | TiDB/PG |
