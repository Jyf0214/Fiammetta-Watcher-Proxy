...
| **Vercel / EdgeOne** | Serverless functions handle everything | TiDB / MySQL / MariaDB / PostgreSQL (remote) | HTTP endpoints + external scheduler (Vercel Cron requires Pro) |
| **Node.js standalone** | Full server on your own machine | TiDB / MySQL / MariaDB / PostgreSQL | HTTP endpoints + system cron |
| **Docker** | Containerized deployment | TiDB / MySQL / MariaDB / PostgreSQL | In-container timer, automatic |
...
| Database | D1 (default, zero config) | TiDB/MySQL/MariaDB/PostgreSQL (remote) | TiDB/MySQL/MariaDB/PostgreSQL (remote) | TiDB/MySQL/MariaDB/PostgreSQL |
...
| Pure MySQL | `DB_TYPE=mysql` + `DATABASE_URL` (`mysql://`) | Non-Cloudflare (Vercel / EdgeOne / Node.js / Docker) |
| MariaDB | `DB_TYPE=mariadb` + `DATABASE_URL` (mariadb protocol) | Non-Cloudflare (Vercel / EdgeOne / Node.js / Docker) |
