...
|------------|-------|
| Node.js | 22.x |
| Database | TiDB (`tidb`), pure MySQL (`mysql`), MariaDB (`mariadb`) or PostgreSQL (`pg`), remotely accessible |
...
::: warning Key points
- If `DB_TYPE` is omitted it is inferred from `DATABASE_URL` (`mysql://` → `mysql`, `mariadb://` → `mariadb`, `postgresql://` → `pg`), but setting it explicitly is recommended
