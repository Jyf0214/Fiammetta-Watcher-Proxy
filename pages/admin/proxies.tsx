...
export default function ProxiesPage() {
  ...
  const columns: TableColumnsType<ProxyItem> = [
    {
      title: t("proxy.address"), dataIndex: "address", key: "address", ellipsis: true,
      render: (v: string) => <span className="font-mono text-xs">{v.replace(/\/\/.*@/, "//***@")}</span>,
    },
    {
      title: t("proxy.pool"), key: "poolName", width: 140,
      render: (_: unknown, r: ProxyItem) => r.pool?.name || <span className="text-zinc-400">{t("proxy.no_pool") || "未分组"}</span>,
    },
    {
      title: t("common.status"), key: "status", width: 100, align: "center",
    ...
    {
      title: t("proxy.ban_count"), dataIndex: "banCount", key: "banCount", width: 80, align: "center",
    },
    {
      title: t("common.actions"), key: "actions", fixed: "right", width: 180, align: "center",
      render: (_: unknown, r: ProxyItem) => (
  ...
  return (
    <AdminLayout>
      <PageContainer>
        ...
        <ProCard
          extra={
            <div className="flex items-center gap-3">
              <span className="text-sm text-zinc-500 dark:text-zinc-400">{t("common.total")}: {proxies.length}</span>
              <Select placeholder={t("proxy.filter_pool") || "按代理池筛选"} allowClear className="w-40"
                onChange={(v) => setFilterPool(v)} options={pools.map((p) => ({ value: p.id, label: p.name }))} />
            </div>
          }
        >
          <ResponsiveTable columns={columns} dataSource={proxies} rowKey="id" loading={loading}
            pagination={{ pageSize: 20, showTotal: (total) => t("common.pagination_total", { count: total }) }}
            scroll={{ x: 700 }}
          />
        </ProCard>

        <Modal title={t("proxy.create_proxy") || "添加代理"} open={modalOpen}
          onCancel={() => { setModalOpen(false); form.resetFields(); }}
          onOk={handleCreate} confirmLoading={submitting} centered width={520} style={{ maxWidth: "90vw" }}
        >
          <Form form={form} layout="vertical">
            <Form.Item name="address" label={t("proxy.address")} rules={[{ required: true }]}
              tooltip="http://user:pass@host:port 或 socks5://host:port"
            >
              <Input placeholder="http://127.0.0.1:7890" className="font-mono text-xs" />
            </Form.Item>
            <Form.Item name="poolId" label={t("proxy.pool")}>
        ...
        <Modal title={t("proxy.import") || "批量导入代理"} open={importModalOpen}
          onCancel={() => { setImportModalOpen(false); setImportText(""); setImportPoolId(undefined); }}
          onOk={handleImport} confirmLoading={importing} centered width={560} style={{ maxWidth: "90vw" }}
        >
          <div className="space-y-4">
            <div className="text-sm text-zinc-500 dark:text-zinc-400">
              每行一条，格式：<code className="bg-zinc-100 dark:bg-zinc-800 px-1 rounded">IP:端口:账号:密码</code>
              <br />同一 IP、端口、账号、密码的代理将被覆盖。
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">{t("proxy.pool")}</label>
              <Select placeholder={t("proxy.select_pool") || "选择代理池"} className="w-full" value={importPoolId}
                onChange={setImportPoolId} allowClear options={pools.map((p) => ({ value: p.id, label: p.name }))} />
            </div>
            <div>
