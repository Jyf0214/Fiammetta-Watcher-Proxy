...
export default function ProxyPoolsPage() {
  ...
  const openCreateForm = () => { setEditing(null); form.resetFields(); setModalOpen(true); };

  const openEditForm = (pool: PoolItem) => {
    setEditing(pool); form.setFieldsValue({ name: pool.name }); setModalOpen(true);
  };

  const closeForm = () => { setModalOpen(false); setEditing(null); form.resetFields(); };
  ...
  const columns: TableColumnsType<PoolItem> = [
    { title: t("proxy_pool.name"), dataIndex: "name", key: "name", width: 200 },
    {
      title: t("proxy_pool.proxy_count"), dataIndex: "proxyCount", key: "proxyCount", width: 100, align: "center",
  ...
  return (
    <AdminLayout>
      <PageContainer>
        ...
        />
        <ProCard>
          <ResponsiveTable columns={columns} dataSource={pools} rowKey="id" loading={loading}
        ...
        <Modal
          ...
          open={modalOpen} onCancel={closeForm} onOk={handleSubmit}
          confirmLoading={submitting} centered width={420} style={{ maxWidth: "90vw" }}
        >
          <Form form={form} layout="vertical">
            <Form.Item name="name" label={t("proxy_pool.name")} rules={[{ required: true }]}>
