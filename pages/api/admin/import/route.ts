...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (body.proxyPools && Array.isArray(body.proxyPools)) {
      result.details.proxyPools = await importProxyPools(db, body.proxyPools);
    }

    if (body.platforms && Array.isArray(body.platforms)) {
      result.details.platforms = await importPlatforms(db, body.platforms);
    }

    if (body.modelMaps && Array.isArray(body.modelMaps)) {
      result.details.modelMaps = await importModelMaps(db, body.modelMaps);
    }

    if (body.proxies && Array.isArray(body.proxies)) {
      result.details.proxies = await importProxies(db, body.proxies);
    }

    if (body.plans && Array.isArray(body.plans)) {
      result.details.plans = await importPlans(db, body.plans);
    }

    if (body.apiKeys && Array.isArray(body.apiKeys)) {
      result.details.apiKeys = await importApiKeys(db, body.apiKeys);
    }

    if (body.configs && Array.isArray(body.configs)) {
      result.details.configs = await importConfigs(db, body.configs);
    }
