 ...
 *
 * 功能：
 *   1. 根据 DB_TYPE 环境变量只生成需要的 Prisma Client（避免打包多余 WASM）
 *   2. MySQL/PG 方言额外执行 prisma db push 同步表结构
 *   3. D1 由 Python 部署脚本单独处理建表，不在此处 push
 *
 * 生成目录：
 *   - 所有方言统一输出到 src/generated/client/
 *
 * 使用方式：
...
const GENERATED_DIR = resolve(GENERATED_ROOT, "client");

/** 方言配置：schema 文件 + db push 标记 */
const DIALECTS = {
  d1: { name: "D1", file: "prisma/schema.d1.prisma", needsPush: false },
  tidb: { name: "MySQL", file: "prisma/schema.mysql.prisma", needsPush: true },
  pg: { name: "PostgreSQL", file: "prisma/schema.pg.prisma", needsPush: true },
  hyperdrive: { name: "PostgreSQL", file: "prisma/schema.pg.prisma", needsPush: true },
};
...
console.log(`🎉 Prisma Client 就绪（${dialect.name}，输出到 src/generated/client/）`);
