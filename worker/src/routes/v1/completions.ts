/**
 * POST /v1/completions — Completions 代理接口
 *
 * 兼容 OpenAI SDK 的 Completions 端点，处理 prompt 字段（而非 messages）。
 * 从 chat-completions 模式迁移，结构保持一致。
 */
...
export async function completions(c: Context<{ Bindings: Env }>) {
  ...
  try {
          ...
          try {
            ...
            if (totalTokens > 0) {
              ...
              if (!tpmResult.allowed) {
                console.warn(`[completions] 流式响应 平台 ${route.platform.id} TPM 已超限`, { totalTokens, tpmLimit: route.platform.tpmLimit });
              }
              const apiKeyTpmResult = await recordApiKeyTokens(c.env, apiKey.id, effectiveTpmLimit, totalTokens);
              if (!apiKeyTpmResult.allowed) {
                console.warn(`[completions] 流式响应 API Key ${apiKey.id} TPM 已超限`, { totalTokens, tpmLimit: effectiveTpmLimit });
              }
            }
          } catch (dbError) {
            console.error("[completions] flush 阶段数据库操作失败:", dbError);
          }
        },
      });
    ...
    try {
      ...
      if (totalTokens > 0) {
        const tpmResult = await recordPlatformTokens(c.env, route.platform.id, route.platform.tpmLimit, totalTokens);
        if (!tpmResult.allowed) {
          console.warn(`[completions] 非流式 平台 ${route.platform.id} TPM 已超限`, { totalTokens, tpmLimit: route.platform.tpmLimit });
        }
        const apiKeyTpmResult = await recordApiKeyTokens(c.env, apiKey.id, effectiveTpmLimit, totalTokens);
        if (!apiKeyTpmResult.allowed) {
          console.warn(`[completions] 非流式 API Key ${apiKey.id} TPM 已超限`, { totalTokens, tpmLimit: effectiveTpmLimit });
        }
      }
    } catch (txErr) {
      console.error("[completions] token扣减失败:", txErr);
    }
